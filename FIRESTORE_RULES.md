...
### 문제 1: 전체 랭킹이 안 보이는 경우

**증상:**
- 소셜 탭의 그룹/고시반 랭킹은 보이는데 전체 랭킹이 안 보임
- 콘솔 에러: `❌ [Ranking] 스냅샷 로드 실패: FirebaseError: Missing or insufficient permissions.`

**원인:**
1. **Firestore 보안 규칙 미배포**: `ranking_cache` 읽기 권한이 없음
2. **스냅샷 데이터 미생성**: `ranking_cache/snapshot` 문서가 존재하지 않음

**해결 방법:**

#### 1단계: Firestore 보안 규칙 배포

1. **[Firebase Console](https://console.firebase.google.com/)** 접속
2. **프로젝트 선택**
3. 좌측 메뉴 **"Firestore Database"** 클릭
4. 상단 탭 **"규칙(Rules)"** 클릭
5. 에디터에서 다음 규칙이 있는지 확인:
   ```javascript
   match /ranking_cache/{document} {
     allow read: if isAuthenticated();
     allow write: if false;
   ...
   ```
6. **없으면** → 저장소의 `firestore.rules` 파일 전체 내용을 복사해서 붙여넣기
7. **"게시(Publish)"** 버튼 클릭 ⚠️

#### 2단계: 스냅샷 데이터 생성 확인

보안 규칙 배포 후에도 전체 랭킹이 안 보이면, 스냅샷 데이터가 없는 것입니다.

**Firestore에서 확인:**
1. Firebase Console → **Firestore Database**
2. **데이터(Data)** 탭
3. `ranking_cache` 컬렉션 → `snapshot` 문서가 있는지 확인

**없으면 스냅샷 생성:**

**방법 1: Node.js 스크립트 실행 (로컬)**
```bash
cd admin-scripts
node generate-ranking-snapshot.js
```

**방법 2: Cloud Functions 배포 (서버리스)**
```bash
firebase deploy --only functions
```

배포 후 6시간마다 자동으로 스냅샷이 생성됩니다.

**방법 3: 수동 트리거 (HTTP 요청)**
```bash
# Cloud Functions 배포 후
curl -X POST https://asia-northeast3-[PROJECT-ID].cloudfunctions.net/manualGenerateRankingSnapshot
```

---

### 문제 2: 그룹원 프로필/업적 포인트가 안 보이는 경우

**증상:**
- 그룹원 타일 호버 시 말풍선이 안 뜸
- 업적 포인트가 0점으로 표시됨
- 콘솔 에러: `⚠️ [GroupMembers] {userId} 프로필 로드 실패: FirebaseError: Missing or insufficient permissions.`

**원인:**
- `users/{userId}` 읽기 권한이 본인만 가능하도록 설정되어 있음
...
3. `users/{userId}` 규칙을 다음과 같이 변경:

**변경 전:**
```javascript
match /users/{userId} {
  allow read: if isOwner(userId);  // ❌ 본인만 읽기 가능
  allow write: if isOwner(userId);
}
```
...
```javascript
match /users/{userId} {
  allow read: if isAuthenticated();  // ✅ 모든 인증된 사용자 읽기 가능
  allow write: if isOwner(userId);
}
```
...
**증상:**
- 업적을 여러 개 달성했는데 포인트가 0점으로 표시됨
- 콘솔에서 `unlocked=false`로 나옴

**원인:**
- 업적 데이터 구조 변경: `unlocked` 필드 → `unlockedAt` 필드 사용

**해결 방법:**

코드는 이미 수정되었습니다. Firestore 보안 규칙 배포 후 페이지를 새로고침하면 자동으로 해결됩니다.
...
3. 로그 확인:
   ```
   ✅ [GroupMembers] {userId} 업적 점수: 100점 (5개 업적)
   ```
