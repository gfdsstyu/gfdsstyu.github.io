...
- ✅ Phase 5.3: index.html 최종 정리 완료 (358줄 감소)
- ✅ **Phase 6.1: 저리스크 UI 함수 window.* 제거 (26개)**
- ✅ **Phase 6.2: 날짜 변수 window.* 제거 (13개)** ⬅️ 최신
- ✅ 버그 수정 14건 완료 (캘린더 월간 이동 버튼 포함)
- ✅ 기능 추가: 문제목록 정렬 개선, 퀴즈 UI 출처 표시

### 🎯 다음 목표
1. **완료**: ✅ index.html 1,000줄 이하 달성! (현재: 801줄)
2. **진행 중**: Phase 6 - 모듈 간 window.* 의존성 제거 (39개 / ~200개 완료)
3. **다음**: 순환 의존성 해결 (EventBus 패턴)
...
## 🎯 Phase 6: 모듈 간 window.* 의존성 제거 (진행 중)

### ✅ Phase 6.1: 저리스크 UI 함수 window.* 제거 (완료)
**일시**: 2025-11-08
**커밋**: `c541e74`

**제거된 window.* 사용 (26개)**:
- `window.showToast`: 15회 → `showToast` (직접 import)
- `window.closeDrawer`: 1회 → `closeDrawer` (직접 import)
- `window.closeReportModal`: 2회 → `closeReportModal` (직접 import)
- `window.openApiModal`: 4회 → `openApiModal` (직접 import)
- `app.js`에서 window.* 정의 4개 제거

**수정된 파일 (5개)**:
1. `js/features/settings/settingsCore.js` - closeReportModal import 추가
2. `js/features/report/reportCore.js` - showToast, closeDrawer import 추가
3. `js/features/report/analysis.js` - showToast, openApiModal import 추가
4. `js/features/quiz/grading.js` - openApiModal import 추가
5. `js/app.js` - window.* 정의 4개 제거

**성과**:
- 총 26개 window.* 제거 (22회 사용 + 4개 정의)
- 모듈 간 명시적 의존성 확립
- 리스크: 낮음 🟢 (저리스크 UI 헬퍼 함수만)
- 소요 시간: 약 1시간

---

### ✅ Phase 6.2: 날짜 변수 window.* 제거 (완료)
**일시**: 2025-11-08
**커밋**: `9ddf237`

**제거된 window.* 사용 (13개)**:
- `window.statsRefDate`: 7회 → `getStatsRefDate()` / `setStatsRefDate()`
- `window.calRefDate`: 6회 → `getCalRefDate()` / `setCalRefDate()`

**수정된 파일 (3개)**:
1. `js/core/storageManager.js`
   - `calRefDate` 변수 추가 및 export
   - `getCalRefDate()`, `setCalRefDate()` 함수 추가
2. `js/features/calendar/calendarCore.js`
   - 읽기: `window.statsRefDate` → `getStatsRefDate()` (3회)
   - 읽기: `window.calRefDate` → `getCalRefDate()` (1회)
   - 쓰기: `window.statsRefDate = value` → `setStatsRefDate(value)` (2회)
   - 수정: `window.statsRefDate.setDate()` → getter로 가져온 후 수정 (2회)
   - 수정: `window.calRefDate.setMonth()` → getter로 가져온 후 수정 (2회)
   - `initCalendarListeners()` 리팩토링 (window.el 제거)
3. `index.html`
   - `window.calRefDate` 정의 제거 (storageManager로 이동)

**성과**:
- 총 13개 window.* 제거 (날짜 변수)
- storageManager를 통한 중앙화된 날짜 관리
- 리스크: 중간 🟡 (날짜 동기화 관련)
- 소요 시간: 약 1시간

---

### 🔄 Phase 6.3: 순환 의존성 해결 (진행 예정)
**상태**: 계획 완료, 실행 대기
**분석 문서**: `CIRCULAR_DEPENDENCY_ANALYSIS.md`

**발견된 순환 의존성 (1개)**:
```
quizCore.js ⇄ filterCore.js
├─ quizCore → filterCore: getFilteredByUI() 2회
└─ filterCore → quizCore: reloadAndRefresh() 4회
```

**해결 방법**: EventBus (Pub/Sub 패턴) 도입
**예상 소요**: 2.5시간
**리스크**: 중간 🟡

---

### ⏸️ Phase 6.4: UI 함수 리팩토링 (대기)
**대상**:
- `window.refreshPanels`: 18회
- `window.reloadAndRefresh`: 18회
- `window.updateSummary`: 12회

**예상 소요**: 5-7시간 (순환 의존성 해결 후)
**리스크**: 높음 🔴

---

### ⏸️ Phase 6.5: 상태 변수 마이그레이션 (대기)
**대상**:
...
- ✅ Phase 6.1 완료 (저리스크 UI 함수 26개 제거)
- ✅ Phase 6.2 완료 (날짜 변수 13개 제거)

**다음 단계**:
1. **Phase 6.3 (진행 중)**: 순환 의존성 해결
   - EventBus 패턴 도입
   - quizCore ⇄ filterCore 순환 제거
   - 예상 소요: 2.5시간
   - 리스크: 중간 🟡
2. **Phase 6.4-6.5 (선택 사항)**: 나머지 window.* 의존성 제거
   - 위험도: 높음 🔴
   - 단계적 접근 필요
   - 예상 소요: 12-17시간
3. **PR 생성 및 머지**
...
**작성일**: 2025-11-08
**브랜치**: `claude/refactor-global-bridge-phase-5-011CUteDeiBPahXqdwr6aGce`
**전체 진행률**: 96% (Phase 6.2 완료, 24개 모듈 생성)
**현재 index.html**: 801줄 (시작 대비 -83.3%)
**window.* 제거**: 39개 / ~200개 (약 20% 완료)
