...
1. **회계감사기준 (KSA)**: 2025년 개정 사항 포함 - **한국 회계감사기준(KSA) 기준으로 답변** (ISA가 아님)
...
학생의 질문에 답변해주세요.`;
    ...
    // Quiz 모드 (문제 있지만 채점 결과 없음 - 학습/연습용)
    const hasGrading = this.userAnswer && this.feedback && this.feedback.score !== undefined;
    if (!hasGrading) {
      return `# 당신의 역할
KICPA 2차 시험 출제 의도와 채점 기준을 완벽히 파악한 고득점 합격자이자 튜터입니다. 문제 풀이와 기준서 암기를 돕는 것이 목표입니다.
...
- **원칙**: KSA(한국 회계감사기준) 번호와 문단을 명시하고, 핵심 문장을 인용구(>)로 원문 그대로 표기
- **주의**: 조사 하나까지 정확하게 표기하여 학생이 눈으로 익힐 수 있게 합니다
- **기준서 유형**: 한국 회계감사기준(KSA)을 기준으로 하며, 2025년 개정사항을 반영합니다
...
## 5. 문제 풀이 전략
학생이 이 문제를 스스로 풀 수 있도록 접근법과 사고 과정을 단계별로 안내합니다.

학생의 질문에 답변해주세요.`;
    }

    // Exam 모드 (채점 완료된 문제 - 오답 분석용)
    return `# 당신의 역할
