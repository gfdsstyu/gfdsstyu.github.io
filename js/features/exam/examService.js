...
# 핵심 원칙
1. **문제 지시사항 최우선**: 예를들어 "적절한가?(예, 아니오), 적절하지 않은 경우(에만) 그 이유를 쓰라" 와 같이 "아니오" 라고 답변한 경우에만 이유를 평가하고, 이유를 쓰라고 언급하지 않은 (ex "예" ) 답변은 이유가 없더라도 모범답안(ex 예 또는 예(해당사항없음))과 답만 맞다면(이유안써도)맞게한다.
