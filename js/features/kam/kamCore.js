...
## JSON 응답 형식
**중요:** 점수를 매기기 전에 먼저 `reasoning` 필드에서 분석 과정을 서술하세요. 이는 채점의 정확도와 타당성을 높입니다.

{
  "reasoning": "먼저 모범 답안의 핵심 요소를 분석합니다. 모범 답안은 [Entity-Specific 요소 1], [위험 요소 1], [위험 요소 2]를 포함하고 있습니다. 사용자의 답안을 보니 [Entity-Specific 요소 1]은 잘 포착했으나, [위험 요소 2]가 누락되었습니다. 또한 '[사용자 표현 인용]' 부분에서 일반적 기재 패턴이 발견됩니다. 따라서...",
  "score": 점수 (0-100),
...
- ✅ **경영진 주장(Assertion) 수준 평가:** 사용자가 작성한 절차가 입증하려는 경영진 주장이 명확한가?
  - **실재성(Existence/Occurrence)**: 검증 대상이 실제로 존재하는가? 거래가 실제 발생했는가?
    - 관련 키워드: "실사", "현물확인", "거래처 확인", "은행조회", "계약서 대조", "발생 근거 확인"
  - **완전성(Completeness)**: 모든 거래/항목이 누락 없이 기록되었는가?
    - 관련 키워드: "전수 대조", "시스템 집계", "차액 분석", "미기록 항목 검토", "예외 거래 검토"
  - **평가 및 배분(Valuation and Allocation)**: 금액이 적절하게 평가되었는가?
    - 관련 키워드: "민감도 분석", "재계산", "외부 시장자료", "공정가치 평가", "손상 검토", "가정 타당성", "할인율"
  - **권리와 의무(Rights and Obligations)**: 자산/부채에 대한 권리와 의무가 실재하는가?
    - 관련 키워드: "소유권 확인", "계약 조건 검토", "담보/저당권 확인", "의무 조항 검토"
  - **표시 및 공시(Presentation and Disclosure)**: 재무제표 표시와 주석 공시가 적절한가?
    - 관련 키워드: "주석 검토", "공시 적정성", "회계기준 준수", "분류 적정성", "주요 가정 공시"

  **참고:** 하나의 감사 절차가 여러 주장을 동시에 검증할 수 있습니다 (예: "외부 전문가 평가보고서 검토" → 평가+공시). 절차의 성격상 어떤 주장을 검증하는지 드러나는가를 확인하세요.

### 5-1단계: Why-How 연계성 평가 (Linkage)
**핵심 질문:** 사용자가 작성한 감사 절차가 선정 이유(Why)에서 식별된 구체적 위험에 직접 대응하는가?

**평가 방법:**
1. 선정 이유(Why)에서 언급된 구체적 위험 요소를 확인하세요
   - 예: "미래 현금흐름 예측의 불확실성", "복잡한 계약 조건", "경영진의 유의적 판단"
2. 사용자의 감사 절차가 이 위험 요소에 직접 대응하는지 확인하세요
   - 예시 (Good): Why에 "미래 현금흐름 예측의 불확실성" → How에 "민감도 분석, 과거 실적 대조" → ✅ 연계성 우수
   - 예시 (Bad): Why에 "복잡한 정산 구조" → How에 "단순 실사" → ❌ 연계성 미흡 (IT 시스템 통제 테스트가 더 적절)
3. 연계성이 미흡한 경우, 피드백에서 명시적으로 지적하세요
   - "선정 이유에서 '[위험 요소 X]'를 언급하셨는데, 이에 대응하는 구체적인 감사 절차가 누락되었습니다."
...
{
  "reasoning": "먼저 모범 답안의 절차를 분석합니다. 모범 답안은 [절차 1: 내부통제 테스트], [절차 2: 민감도 분석], [절차 3: 전문가 활용] 등 총 X개의 절차를 포함하고 있습니다. 사용자는 Y개의 절차를 작성했으며, 이 중 [절차 1]은 Strong Match, [절차 2]는 Partial Match입니다. 하지만 모범 답안의 핵심 절차인 [절차 3]이 누락되었습니다. 따라서...",
  "score": 점수 (0-100),
...
`;
  ...
  async callGeminiForKAM(systemPrompt, userQuery, apiKey, model = 'gemini-2.5-flash') {
    // 모델 선택 가이드:
    // - gemini-2.5-flash: 빠른 응답, 일반적인 평가에 적합 (기본값)
    // - gemini-2.0-flash: 구버전 Flash 모델
    // - gemini-2.5-pro: 더 깊이 있는 분석이 필요한 경우 (느리지만 정확)
    // - gemini-3-pro: 최신 Pro 모델 (실험적, 더 깊은 추론)
    const MODEL_MAP = {
    ...
    // Pro 모델은 더 정밀한 추론을 위해 temperature를 약간 조정
    const isPro = model.includes('pro');
    const temperature = isPro ? 0.2 : 0.3; // Pro는 더 일관성 강조
    ...
    const payload = {
      ...
      generationConfig: {
        ...
        topP: isPro ? 0.95 : 0.8, // Pro는 더 넓은 탐색
        topK: isPro ? 64 : 40 // Pro는 더 많은 옵션 고려
      }
    };
  ...
  calculateFinalScore(whyResult, howResult) {
    // 단독 평가 지원: 하나만 응시했을 경우 해당 점수를 100% 반영
    if (!whyResult && howResult) {
      return {
        finalScore: howResult.score,
        whyScore: null,
        howScore: howResult.score,
        overallFeedback: this.generateOverallFeedback(howResult.score, null, howResult),
        evaluationType: 'how-only'
      };
    }

    if (whyResult && !howResult) {
      return {
        finalScore: whyResult.score,
        whyScore: whyResult.score,
        howScore: null,
        overallFeedback: this.generateOverallFeedback(whyResult.score, whyResult, null),
        evaluationType: 'why-only'
      };
    }

    // 둘 다 응시했을 경우: Why 40%, How 60% 가중치
