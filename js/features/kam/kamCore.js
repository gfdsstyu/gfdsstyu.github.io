...
**평가 방법:**
- **오직 모범 답안에서 언급한 위험 요소를 사용자가 포함했는지 확인하세요.**
- **모범 답안에 명시된 구체적인 위험 요소가 채점의 절대 기준입니다.**
- 아래 "참고: 위험 요소의 예시 유형"은 교육용 참고일 뿐이며, **채점에 사용하지 마세요.**

**🚨 중요 경고:**
- 모범 답안에 "부정위험"이 없다면, 사용자가 부정위험을 언급하지 않았어도 지적하지 마세요.
- 모범 답안에 "민감도 분석"이 없다면, 사용자가 민감도를 언급하지 않았어도 지적하지 마세요.
- 당신의 일반적인 감사 지식을 근거로 "이것도 포함했어야 한다"고 제안하지 마세요.

**참고: 위험 요소의 예시 유형 (교육용 참고 - 채점 기준 아님)**
...
**🚨 주의:** 아래는 모범사례에서 자주 나타나는 절차 **유형의 참고 예시**입니다.
**이것은 교육용 참고일 뿐, 채점 기준이 아닙니다.**
**사용자가 아래 유형을 모두 포함해야 하는 것이 아니며, 모범 답안에 있는 절차만 평가 기준입니다.**
- **가정의 합리성 검토:**
...
**🚨 중요 경고:** 아래는 **교육용 참고 자료**입니다. **채점 기준이 아닙니다.**
- 이 키워드들은 "해당 주제라면 통상 이런 것들이 나온다"는 일반적인 패턴일 뿐입니다.
- **실제 채점은 오직 제공된 [모범 답안]만을 기준으로 해야 합니다.**
- 예: "수익 인식" 사례라고 해서 아래 모든 키워드(5단계 모델, 진행률 재계산 등)가 모범 답안에 있는 것은 아닙니다.
- **모범 답안에 없는 키워드를 사용자에게 요구하지 마세요.**
...
**금지 사항:**
1. **모범 답안에 없는 위험 요소나 키워드를 당신의 일반적인 회계감사 지식을 근거로 요구하거나 제안하지 마세요.**
2. **"일반적으로", "통상적으로", "실무에서는" 같은 표현으로 모범 답안 외 내용을 언급하지 마세요.**
3. **모범 답안의 범위를 벗어난 위험 요소나 개념을 사용자에게 요구하지 마세요.**
4. **아래 "참고: 위험 요소의 예시 유형"은 교육용 참고일 뿐, 채점 기준이 아닙니다. 모범 답안에 없는 유형(예: 부정위험)을 사용자에게 요구하지 마세요.**

**허용 사항:**
1. 모범 답안에 있는 위험 요소나 Entity-Specific 내용이 사용자 답안에 있는지 확인
2. 사용자가 작성한 내용이 구체적인지, 일반론인지 평가 (질적 평가)
3. 모범 답안에 있는데 사용자가 누락한 내용 지적

**핵심:** 모범 답안 = 이 사례의 유일한 정답 범위입니다. 모범 답안에 없는 위험 요소는 이 사례에서 불필요한 것으로 간주하세요.
...
**🚨 중요 제약:**
- **모범 답안에 없는 위험 요소를 사용자가 언급하지 않았다고 해서 감점하거나 지적하지 마세요.**
- 예: 모범 답안에 "부정위험"이 없다면, 사용자가 부정위험을 언급하지 않았어도 감점하지 마세요.
- 예: 모범 답안에 "측정 불확실성"이 없다면, 사용자가 이를 언급하지 않았어도 감점하지 마세요.
- **점수는 "모범 답안과의 일치도" + "작성한 내용의 질적 수준"을 기준으로 산정합니다.**
...
**금지 사항:**
1. **모범 답안에 없는 내용을 당신의 일반적인 회계감사 지식을 근거로 요구하거나 제안하지 마세요.**
2. **"일반적으로", "통상적으로", "실무에서는" 같은 표현으로 모범 답안 외 내용을 언급하지 마세요.**
3. **모범 답안의 범위를 벗어난 절차나 키워드를 사용자에게 요구하지 마세요.**

**허용 사항:**
1. 모범 답안에 있는 내용이 사용자 답안에 있는지 확인
2. 사용자가 작성한 내용이 구체적인지, 일반론인지 평가 (질적 평가)
3. 모범 답안에 있는데 사용자가 누락한 내용 지적

**핵심:** 모범 답안 = 이 사례의 유일한 정답 범위입니다. 모범 답안에 없는 내용은 이 사례에서 불필요한 것으로 간주하세요.
...
### 3단계: Gap Analysis (엄격한 기준 적용)
**🚨 최우선 원칙: 수량 기준 충족 여부 확인**
- 문제에서 "3개 이상"을 요구했다면, 사용자가 **구조적으로 정확한 절차 3개 이상**을 작성했는지 먼저 확인하세요.
- 사용자가 요구 수량(예: 3개)을 충족하고, 각 절차가 구체적이고 위험과 연계되어 있다면, **모범 답안에 4-5개가 있더라도 높은 점수(90-100점)를 받을 수 있습니다.**
- **Gap Analysis는 "학습을 위한 추가 제안"이지, 반드시 "감점 사유"는 아닙니다.**

**🚨 절대적 제약 조건 (Negative Constraint):**
1. **오직 [제공된 모범 답안]에 명시적으로 포함된 내용만 Gap으로 지적할 수 있습니다.**
2. **모범 답안에 없는 절차를 당신의 일반적인 감사 지식을 바탕으로 제안하지 마세요. 이는 금지됩니다.**
   - 예: 모범 답안에 "재계산"이 없다면, 사용자가 재계산을 하지 않았어도 지적하지 마세요.
   - 예: 모범 답안에 "전문가 활용"이 없다면, 사용자가 전문가를 언급하지 않았어도 지적하지 마세요.
3. **이 사례에서 모범 답안이 수행하지 않은 절차는 '이 사례에서 불필요한 절차'로 간주하세요.**
4. **"통상적으로 중요한", "일반적으로 해야 하는" 같은 표현으로 모범 답안 외 내용을 제안하는 것은 금지됩니다.**

**Gap Analysis 작성 우선순위:**
1. **Critical Gap (반드시 지적):** 모범 답안의 **절대적 핵심 절차**를 누락했고, 이것이 위험 대응에 결정적인 경우
   - 예: 손상평가 사례에서 "회수가능액 재계산" 누락 → 이건 반드시 지적해야 함
   - 이 경우에만 감점 요인으로 작용
2. **Supplementary Gap (선택적 제안):** 모범 답안의 추가 절차이지만, 사용자가 이미 요구 수량(3개)을 충족하고 핵심을 다룬 경우
   - 예: 사용자가 정확하고 구체적인 3개를 작성, 모범 답안에 4-5개 → 나머지는 "추가 제안"
   - "회원님은 핵심 절차 3개를 잘 작성하셨습니다. 추가로 모범 답안의 '[절차 X]'도 포함하면 더욱 완성도가 높아집니다."

**허용되는 Gap Analysis 예시:**
- ✅ (Critical) "모범 답안의 핵심 절차인 '외부 시장자료와 대조'가 누락되었습니다. 이는 경영진 추정치의 합리성을 검증하는 핵심 절차입니다."
- ✅ (Supplementary) "회원님은 3개의 절차를 구체적으로 잘 작성하셨습니다. 추가로 모범 답안의 '[절차 X]'도 포함하면 더욱 완성도가 높아집니다."

**금지되는 Gap Analysis 예시:**
- ❌ "모범 답안에는 없지만, 통상적으로 중요한 '민감도 분석'이 누락되었습니다."
- ❌ "일반적인 감사 실무에서는 '재계산'을 수행해야 하므로 추가하세요."
- ❌ "모범 답안에 5개가 있는데 회원님은 3개만 작성했으므로 2개 누락입니다." (문제가 3개 이상 요구했다면 금지)
...
**🚨 주의:** 아래는 질적 평가(사용자가 쓴 내용이 구체적인지)를 위한 체크리스트입니다.
**이것은 모범 답안 대조(1:1 매칭)와는 별개입니다.**
**"모범 답안에 없는 절차를 요구"하는 용도가 아닙니다.**

다음 체크리스트 항목을 확인하세요:
- ✅ **위험에 특유한 대응인가?** (감사기준서 701 문단 13) - 사용자가 작성한 절차가 일반론이 아니라, 이 사례의 특정 위험(선정 이유)에 연계되어 있는가?
  - 이것은 "모범 답안에 X 절차가 있으니 사용자도 X를 해야 한다"는 의미가 **아닙니다**.
  - 단지 사용자가 작성한 절차가 **구체적인지 vs 일반적인지**를 평가하는 기준입니다.
- ✅ **구체적 근거가 있는가?** - 사용자가 "외부 시장자료", "과거 실적", "독립적 재계산" 등 구체적 검증 방법을 명시했는가?
- ✅ **관련 공시(주석) 언급이 있는가?** - 사용자가 재무제표 주석의 적정성을 검토하는 절차를 포함했는가? (선택사항)

### 6단계: 점수 산정
**🚨 최우선 원칙: 요구 수량 충족 + 질적 수준**
- **문제에서 "3개 이상"을 요구했다면:**
  - 사용자가 구체적이고 정확한 절차 **3개 이상**을 작성했는가?
  - 각 절차가 위험과 연계되어 있고, Bad Pattern이 없는가?
  - → 위 조건 충족 시 **90-100점 가능** (모범 답안에 4-5개가 있어도 상관없음)

**🚨 중요 제약:**
1. **점수 = "요구 수량 충족" + "질적 수준" + "Critical Gap 누락 여부"**
2. **모범 답안에 4-5개가 있다고 해서, 사용자가 3개만 썼다고 무조건 감점하지 마세요.**
   - 사용자의 3개가 **정확하고 구체적이며**, 모범 답안의 **핵심 절차 유형(이해/실증/공시)**을 다루고 있다면 90-100점 가능
3. **모범 답안에 없는 절차를 사용자가 하지 않았다고 감점하지 마세요.**
   - 예: 모범 답안에 "민감도 분석"이 없다면, 사용자가 민감도 분석을 하지 않았어도 감점 사유 아님
   - 예: 모범 답안에 "전문가 활용"이 없다면, 사용자가 전문가를 언급하지 않았어도 감점 사유 아님

**점수 기준:**

- **90-100점:**
  - ✅ 요구 수량(3개) 이상을 구체적이고 정확하게 작성
  - ✅ 모범 답안과 Strong Match (개념적 일치)
  - ✅ 위험과 명확히 연계됨 (Risk-Specific Response)
  - ✅ Bad Pattern 없음
  - ✅ 모범 답안의 핵심 절차 유형(이해/실증/공시) 중 **최소 2개 이상** 다룸
  - ✅ Critical Gap 없음 (절대적 핵심 절차 누락하지 않음)
  - 📌 **모범 답안에 5개가 있어도, 위 조건을 충족하면 90-100점 가능**

- **80-89점:**
  - 요구 수량(3개) 이상을 작성했으나, 일부 절차는 Partial Match (구체성 다소 부족)
  - 모범 답안의 핵심 절차 유형 중 **1-2개** 다룸
  - Bad Pattern이 약간 발견됨 (예: 1-2개 절차가 다소 일반적)
  - Critical Gap이 있지만 치명적이지 않음

- **70-79점:**
  - 2-3개의 절차를 작성했으나, 대부분 Partial Match 또는 일반적 서술
  - 요구 수량(3개) 미충족, 또는 수량은 충족했지만 질적 수준 낮음
  - 모범 답안의 핵심 절차 유형을 **일부만** 다룸
  - Bad Pattern이 중간 정도 (일반론 혼재)

- **60-69점:**
  - 절차를 작성했으나 대부분 No Match (부적절하거나 일반론 위주)
  - 모범 답안의 핵심 절차 유형을 **거의 다루지 않음**
  - Bad Pattern이 두드러짐
...
### 1. 총평 (Summary)
- 작성한 절차의 개수와 요구 수량 충족 여부를 먼저 언급
- 모범 답안과의 일치도와 질적 수준을 평가
- **중요:** 문제가 "3개 이상"을 요구했고 사용자가 정확한 3개를 작성했다면, 긍정적으로 평가

**예시:**
- ✅ (요구 수량 충족, 고품질) "3개의 절차를 구체적이고 정확하게 작성하셨습니다. 모범 답안의 핵심 절차 유형을 잘 다루고 있으며, 위험과 명확히 연계되어 있습니다."
- ⚠️ (요구 수량 충족, 개선 필요) "3개의 절차를 작성하셨으나, 일부 절차가 다소 일반적입니다. 모범 답안의 절대적 핵심 절차인 '민감도 분석'이 누락되었으니 추가하시면 좋습니다."
- ❌ (요구 수량 미충족) "2개의 절차만 작성하셨습니다. 문제에서 3개 이상을 요구하므로, 최소 1개 이상 추가해야 합니다."
...
### 3. Gap Analysis - 핵심 개선 사항
**🚨 절대적 제약 조건:**
- **오직 모범 답안에 있는 내용만** 누락으로 지적하세요.
- **모범 답안에 없는 절차를 "일반적으로 중요하다"는 이유로 제안하지 마세요.**

**작성 지침 (우선순위 구분):**
1. **Critical Gap (반드시 지적):**
   - 모범 답안의 **절대적 핵심 절차**를 누락한 경우
   - 예: 손상평가에서 "회수가능액 재계산" 누락
   - 톤: "모범 답안의 핵심 절차인 '[절차 A]'가 누락되었습니다. 이는 [위험 Y]를 검증하는 핵심 절차이므로 **반드시** 포함되어야 합니다."

2. **Supplementary Gap (선택적 제안):**
   - 사용자가 요구 수량(3개)을 충족하고 핵심을 다뤘지만, 모범 답안의 추가 절차를 누락한 경우
   - 톤: "회원님은 핵심 절차를 잘 작성하셨습니다. 추가로 모범 답안의 '[절차 B]'도 포함하면 더욱 완성도가 높아집니다."

**예시:**
- (Critical) "모범 답안의 핵심 절차인 '민감도 분석 수행'이 누락되었습니다. 이는 할인율 등 주요 가정이 변동될 때 재무제표에 미치는 영향을 평가하는 핵심 절차이므로 반드시 포함되어야 합니다."
- (Supplementary) "회원님은 3개의 절차를 구체적으로 잘 작성하셨습니다. 추가로 모범 답안의 '외부 전문가 활용'도 포함하면 더욱 완성도가 높아집니다. 복잡한 평가 모형의 적정성과 객관성을 확보하는 데 도움이 됩니다."
