...
export async function handleGrade() {
  ...
  // API 키 확인
  const geminiApiKey = window.getGeminiApiKey?.();
  if (!geminiApiKey) {
  ...
  try {
    ...
    let { score, feedback } = await callGeminiAPI(
      ...
      q.정답,
      geminiApiKey,
      window.getSelectedAiModel?.() || 'gemini-2.5-flash'
    );

    // Lite 모델 감점
    const selectedAiModel = window.getSelectedAiModel?.() || 'gemini-2.5-flash';
    if (selectedAiModel === 'gemini-2.5-flash-lite') {
      score = clamp(score - 7, 0, 100);
    }

    // 힌트 사용 감점
    const activeHintQuestionKey = window.getActiveHintQuestionKey?.();
    ...
    // 점수 저장 (StateManager 사용)
    if (typeof window.setQuestionScores === 'function') {
      window.setQuestionScores(questionScores);
    }
    ...
    // localStorage 저장
    try {
      if (typeof window.saveQuestionScores === 'function') {
        window.saveQuestionScores();
      } else {
...
export async function handleHint(q) {
  ...
  try {
    ...
    if (typeof window.setActiveHintQuestionKey === 'function') {
      window.setActiveHintQuestionKey(qKey);
    }
